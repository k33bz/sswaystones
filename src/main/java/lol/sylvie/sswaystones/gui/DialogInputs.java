...
/**
 * Builders for the native-dialog input controls and the {@link CommandTemplate}
 * action that submits their values into a backend command via {@code $(key)}
 * substitution.
 *
 * <p>
 * The dialog system has no boolean/checkbox control, so booleans are modeled as
 * a two-entry {@link SingleOptionInput} ("On"/"Off") whose selected entry id
 * ("true"/"false") is substituted into the command.
 */
public final class DialogInputs {
    ...
    /**
     * A single-line text input bound to {@code key}, for later {@code $(key)}
     * substitution.
     */
    public static Input text(String key, String label, String initial, int maxLength, int width) {
        return new Input(key, new TextInput(width, Component.literal(label), true, initial == null ? "" : initial,
                maxLength, Optional.empty()));
    }

    /**
     * A boolean modeled as an On/Off single-option picker bound to {@code key}. The
     * submitted value is the string "true" or "false"; {@code initial} pre-selects
     * the matching entry.
     */
    public static Input bool(String key, String label, boolean initial, int width) {
        // On=green / Off=red display text, matching the sgui menu's toggle colors;
        // the submitted ids stay plain "true"/"false".
    ...
    /**
     * One selector entry: submitted id, its colored display text, and whether it
     * starts selected.
     */
    ...
    /**
     * A single-option (cycling) picker bound to {@code key} built from pre-made
     * {@link #entry} entries. The submitted value is the selected entry's id.
     */
    ...
    /**
     * A submit action that runs {@code template} with the dialog's input values
     * substituted in ({@code $(key)} placeholders). {@link ParsedTemplate} has no
     * public constructor, so we decode one through its string codec — the same path
     * the vanilla dialog loader uses.
     */
